...
CREATE TABLE
IF NOT EXISTS PCS_Administrator
(
    username VARCHAR
(256),
    salary numeric,
...
(256),
    category VARCHAR
(256) NOT NULL REFERENCES Pet_Categories
(category) ON
DELETE CASCADE,
    pname VARCHAR(256)
...
-- SELECT SUM
--     (CASE
--         WHEN end_date >= `${end_date}` AND start_date <= `${start_date}` THEN (`${end_date}`::date - `${end_date}`::date + 1) * payment_amt 
--         WHEN end_date <= `${end_date}` AND start_date >= `${start_date}` THEN (end_date::date - start_date::date + 1) * payment_amt
--         WHEN end_date <= `${end_date}` AND end_date >= `${start_date}` AND start_date <= `${start_date}` THEN (end_date::date - `${start_date}`::date + 1) * payment_amt
--         WHEN end_date >= `${end_date}` AND start_date <= `${start_date}` AND start_date >= `${start_date}` THEN (`${end_date}`::date - start_date::date + 1) * payment_amt
--         ELSE 0 
--     END) AS salary 
-- FROM bids
